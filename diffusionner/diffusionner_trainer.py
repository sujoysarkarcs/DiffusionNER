...
# FIX: AdamW was moved out of transformers into torch.optim in transformers>=4.30.
# We try torch.optim first (PyTorch>=2.0), then fall back to transformers.
try:
...
class DiffusionNERTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        # FIX: 'correct_bias' was renamed to 'no_deprecation_warning' and then removed;
        # torch.optim.AdamW does not have this param. Only pass it if using transformers AdamW.
        optimizer_kwargs = dict(lr=args.lr, weight_decay=args.weight_decay)
        try:
            # transformers AdamW accepts correct_bias
            optimizer = AdamW(optimizer_params, correct_bias=False, **optimizer_kwargs)
        except TypeError:
            # torch.optim.AdamW does not accept correct_bias
            optimizer = AdamW(optimizer_params, **optimizer_kwargs)
        ...
        compute_loss = DiffusionNERLoss(input_reader.entity_type_count, self._device, model, optimizer, scheduler,
                                        ...
                                        args.match_boundary_type, args.type_loss, solver=args.match_solver)
        ...
        test_f1 = None
        best_epoch = 0
        extra = dict(epoch=0, updates_epoch=updates_epoch, epoch_iteration=0)
    ...
    def _eval(self, model: torch.nn.Module, dataset, input_reader: JsonInputReader,
        ...
        evaluator = Evaluator(dataset, input_reader, self._tokenizer, self._logger, args.no_overlapping,
                              ...
                              entity_threshold=args.entity_threshold, save_prediction=args.store_predictions)
