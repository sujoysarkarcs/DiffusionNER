...
class DiffusionNERLoss(Loss):
    def __init__(self, entity_type_count, device, model, optimizer, scheduler, max_grad_norm, nil_weight,
                 match_class_weight, match_boundary_weight, loss_class_weight, loss_boundary_weight,
                 match_boundary_type, type_loss, solver):
        self._model = model
        ...
        self.criterion = Criterion(entity_type_count, self.weight_dict, nil_weight, losses,
                                   type_loss=type_loss, match_class_weight=match_class_weight,
                                   match_boundary_weight=match_boundary_weight,
                                   match_boundary_type=match_boundary_type, solver=solver)
    ...
    def compute(self, output, gt_types, gt_spans, entity_masks, epoch, batch=None):
        ...
        pred_logits, pred_left, pred_right, pred_left, pred_right = (
            output["pred_logits"],
            output["pred_spans"][:, :, 0],
            output["pred_spans"][:, :, 1],
            output["pred_left"],
...
class Criterion(nn.Module):
    def __init__(self, entity_type_count, weight_dict, nil_weight, losses, type_loss,
                 match_class_weight, match_boundary_weight, match_boundary_type, solver):
        super().__init__()
        self.entity_type_count = entity_type_count
        self.matcher = HungarianMatcher(cost_class=match_class_weight, cost_span=match_boundary_weight,
                                        match_boundary_type=match_boundary_type, solver=solver)
    ...
    def loss_labels(self, outputs, targets, indices, num_spans):
        ...
        if self.type_loss == "bceloss":
            src_logits = src_logits.view(-1, src_logits.size(2))
            target_classes = target_classes.view(-1)
            target_classes_onehot = torch.zeros([target_classes.size(0), src_logits.size(1)],
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        left_onehot = torch.zeros([target_spans_left.size(0), src_spans_left.size(1)],
                                   dtype=torch.float32).to(device=target_spans_left.device)
        left_onehot.scatter_(1, target_spans_left.unsqueeze(1), 1)

        right_onehot = torch.zeros([target_spans_right.size(0), src_spans_right.size(1)],
                                    dtype=torch.float32).to(device=target_spans_right.device)
