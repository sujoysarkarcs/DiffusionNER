...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: JsonInputReader, text_encoder: BertTokenizer,
                 logger, no_overlapping: bool, no_partial_overlapping: bool, no_duplicate: bool,
                 predictions_path: str, examples_path: str, example_count: int, epoch: int,
                 dataset_label: str, cls_threshold=0, boundary_threshold=0, entity_threshold=0,
    ...
    def eval_batch(self, outputs, batch=None):
        ...
        batch_entity_mask = batch_entity_mask * (batch_entity_spans[:, :, 0] <= batch_entity_spans[:, :, 1]) * \
                            ((batch_entity_left_scores > self._boundary_threshold) | (batch_entity_right_scores > self._boundary_threshold))
        ...
        for i in range(batch_size):
            ...
            if self._input_reader.entity_type_count < 1000 and self._save_prediction:
                ...
                for j in range(entity_left.size(1)):
                    ...
                    decode_entity["pre_entities"].append(dict(
                        entity_left=entity_left[i][j].item(),
                        entity_right=entity_right[i][j].item(),
                        phrase=span_tokens,
            ...
            sample_pred_entities = self._convert_pred_entities(valid_entity_types, valid_entity_spans,
                                                               valid_type_scores + valid_left_scores + valid_right_scores,
                                                               valid_left_scores, valid_right_scores,
                                                               valid_type_scores, doc)
    ...
    def store_examples(self):
        ...
        self._store_examples(sorted(entity_examples[:self._example_count], key=lambda k: k['length']),
    ...
    def _convert_pred_entities(self, pred_types: torch.tensor, pred_spans: torch.tensor,
                               pred_scores: torch.tensor, left_scores, right_scores, type_scores, doc):
        converted_preds = []
        ...
        for i in range(pred_types.shape[0]):
            ...
            decode_entity["entities"].append({
                "start": start, "end": end,
                "entity_type": entity_type.identifier,
    ...
    def _compute_metrics(self, gt_all, pred_all, types, print_results: bool = False):
        ...
        # FIX: sklearn>=1.0 raises UndefinedMetricWarning when there are no predictions
        # for a label. Setting zero_division=0 silences the warning and returns 0 instead
        # of NaN, matching the behaviour expected by the rest of the code.
    ...
    def _store_examples(self, examples: List[Dict], file_path: str, template: str):
        ...
        # FIX: jinja2>=3.0 deprecated passing a string directly to Template() for rendering
        # from a file. Use Environment + FileSystemLoader instead for proper path handling.
        template_dir = os.path.join(SCRIPT_PATH, 'templates')
        env = jinja2.Environment(loader=jinja2.FileSystemLoader(template_dir))
        tmpl = env.get_template(template)
        tmpl.stream(examples=examples).dump(file_path)
