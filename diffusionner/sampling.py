...
def create_train_sample(doc, repeat_gt_entities=100):
    ...
    return dict(encodings=encodings, context_masks=context_masks, seg_encoding=seg_encoding,
                ...
                gt_types=gt_entity_types, gt_spans=gt_entity_spans_token,
                entity_masks=gt_entity_masks, meta_doc=doc)
...
def collate_fn_padding(batch):
    ...
    for key in keys:
        ...
        if key.startswith("image_inputs"):
            if batch[0]["image_inputs"] is None:
                padded_batch["image_inputs"] = None
            else:
                padded_batch["image_inputs"] = dict(
                    (k, torch.cat([s["image_inputs"][k] for s in batch], dim=0))
